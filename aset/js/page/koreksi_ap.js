...
var table;
var url_list = host + "Koreksi_ap/ajax_list/";
var url_edit = host + "Koreksi_ap/ajax_edit/";
var url_hapus = host + "Koreksi_ap/ajax_delete/";
var url_simpan = host + "Koreksi_ap/simpan";
var url_update = host + "Koreksi_ap/ajax_update";
var url_simpan_remark = host +"Koreksi_ap/save_remark";
...
function tambah()
{
    ...
    $('.table-apcorrection tbody').empty();
    $('.table-apcorrection2 tbody').empty();
    add_new_row_store();
    add_new_row_selling();
    reset_button_action();
    reset_file_upload();
...
function ambil_apcorrection(page = "",search = "")
{
    $(".table-apcorrection tbody tr").remove();
    $.ajax({
        url : host+"api/ap_correction/ap_correction",
        type: "GET",
        dataType: "JSON",
        success: function(data){
            ...
            if(data.list_data.length > 0){
                ...
                $.each(data.list_data,function(i,v){
                    ...
                    $(".table-apcorrection tbody").append(item);
                });
                angka();
            } else {
                item = '<tr><td style="text-align:center" colspan="4">Empty Data</td></tr>';
                $(".table-apcorrection tbody").append(item);
            }
        },
        error: function (jqXHR, textStatus, errorThrown){
...
function view(id,page){
    ...
    url = host + "koreksi-ap-view/"+id+"?page=print";
...
function view_print_data(id,page){
    ...
    url = host + "koreksi-ap-view/"+id+"?page="+page;
...
function add_new_row_store(v){
    kolom       = $('.table-apcorrection tbody').find('tr').length + 1;
    id_row      += 1;
    btn_remove  = '';
    ...
    item = 
        '<tr class="dtt'+id_row+'">\
            ...
            <td><i class="icon fa-search remove_row" onclick="branch_modal('+"'ap_correction','.dtt"+id_row+"'"+')" style="cursor:pointer;padding:5px;"></i></td>\
            <td>\
                <input type="text" class="readonly pointer" onclick="branch_modal('+"'ap_correction','.dtt"+id_row+"'"+')" name="BranchName[]" value="'+BranchName+'" placeholder="'+language_app.lb_store_select+'">\
...
function add_new_row_selling(v){
    kolom       = $('.table-apcorrection2 tbody').find('tr').length + 1;
    id_row      += 1;
    btn_remove  = '';
    ...
    apRemark        = '';
    if(v){
        apDetid         = v.BalanceDetID;
        apVendorID      = v.vendorid;
        apVendorName    = v.vendorName;
        apTotal         = parseFloat(v.totalcorrection);
        apRemark        = v.Remark;
    }

    item = 
        '<tr class="dt'+id_row+'" data-position="vendor">\
            <td>\
            ...
            <td>\
                <input type="text" class="readonly pointer" onclick="vendor_modal('+"'.dt"+id_row+"'"+')" name="apVendorName[]" value="'+apVendorName+'" placeholder="select vendor">\
                <input type="hidden" name="apVendorID[]" value="'+apVendorID+'"/>\
                <input type="hidden" name="apDetid[]" value="'+apDetid+'"/>\
            </td>\
            <td><input type="text" class="duit" name="apTotal[]" placeholder="'+language_app.lb_input_nominal+'" value="'+apTotal+'"></td>\
            <td><input type="text" name="apRemark[]" placeholder="'+language_app.lb_remark_input+'" value="'+apRemark+'"></td>\
            <td>'+btn_remove+'</td>\
        </tr>';
    $(".table-apcorrection2 tbody").append(item);
    $('.readonly').attr('readonly', true);
    moneyFormat();
}
...
function delete_data(id){
    swal({   
        ...
        cancelButtonText: language_app.btn_close,   
        closeOnConfirm: false,
        showLoaderOnConfirm: true,   
        closeOnCancel: false }, 
        function(isConfirm){   
...
function edit(id){
    ...
    $('.table-apcorrection tbody').children('tr').remove();
    $('.table-apcorrection2 tbody').children('tr').remove();
    reset_button_action();
    reset_file_upload();
    reset_button_action();
    show_upload_file();
