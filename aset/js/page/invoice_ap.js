...
var url_list 		= host + "invoice_ap/ajax_list/";
var url_edit        = host + "invoice_ap/ajax_edit/";
var url_cancel 		= host + "invoice_ap/cancel/";
var url_simpan 		= host + "invoice_ap/save";
var url_simpan_remark = host + "invoice_ap/save_remark";
...
$(document).ready(function() {
    ...
    if(data_page.id){
        ...
        if(statusid[0] != 1){
            if(statusid[1] == "receipt"){
                get_detail_receipt(data_page.id);
            }
...
function tambah(){
    ...
    $('[name=temp_purchaseno]').val('');
    $('[name=temp_receiveno]').val('');
...
function reset_address_vendor(){
    $('.vreceive .address, .vinvoice .address').val('');
    $('.vreceive .city, .vinvoice .city').val('');
    $('.vreceive .province, .vinvoice .province').val('');
    $('.vreceive #NPWP').val('');
}
// end customer address

// invoice_delivery
function invoice_receive(id){
    vendorid        = $('#CustomerID').val();
    vendorid        = vendorid.split('-');
    crud            = $('[name=crud]').val();
    invoiceno       = $('[name=temp_invoiceno]').val();
    receiveno      = $('[name=temp_receiveno]').val();
...
function setitem(v,page,id){
    ...
    if(page == "invoice"){
        ...
        temp_receiveno = temp_receiveno.split(',');
        if(jQuery.inArray(v.ReceiveNo, temp_receiveno) !== -1){
            checked = 'checked';
        }
        receiveno       = v.ReceiveNo;
        receiveno2      = v.ReceiveNo;
    ...
    receiveno_replace = receiveno.replace(/[/]/g, '-');
    // checkbox
    item  = '<tr class="vd'+receiveno_replace+'">';
    item += '<td><div class="info-warning"></div></td>';
    item += '<td>\
            <input class="cekbox" type="checkbox" onclick="transaction_cek(this)" name="check[]" value="'+receiveno_replace+'" '+checked+'>\
            <input class="disabled" type="hidden" name="detid[]" value="'+detid+'" >\
            </td>';

    // delivery no
    item += '<td>\
            <input class="disabled" type="text" name="receiveno[]" value="'+receiveno+'" >\
            <input class="disabled" type="hidden" name="receiveno2[]" value="'+receiveno2+'" >\
            </td>';
    ...
    // selling no
    item += '<td>\
            <input class="disabled" type="text" name="purchaseno[]" value="'+purchaseno+'" >\
            </td>';
    ...
    // Remark
    item += '<td>\
            <input type="text" name="delRemark[]" placeholder="'+language_app.lb_remark_input+'" value="'+remark+'">\
            </td>';
...
function view(id,page){
    ...
    url = host + "invoice-ap-view/"+id+"?page=print";
...
function view_print_data(id,page){
    ...
    url = host + "invoice-ap-view/"+id+"?page="+page;
            ...
            if(list.OrderType == 1){
                ...
                $.each(detail, function(i, v) {
                    ...
                    if(v.invoiceType == "return"){
                        ...
                        temp_receiveno.push(v.ReceiveNo);
                    }
                });
                $('[name=temp_receiveno]').val(temp_receiveno);
...
function setitem_payable(v,page){
    tbl = ".table-add-product";
    ...
    if(page == "invoice"){
        ...
        temp_purchaseno = $('[name=temp_purchaseno]').val();
        temp_purchaseno = temp_purchaseno.split(',');
    ...
    // checkbox
    item  = '<tr class="vd'+purchaseno+'">';
    item += '<td><div class="info-warning"></div></td>';
    item += '<td>\
            <input class="cekbox" type="checkbox" onclick="transaction_cek(this)" name="check[]" value="'+purchaseno+'" '+checked+'>\
            <input class="disabled" type="hidden" name="detid[]" value="'+detid+'" >\
            </td>';
    ...
    // selling no
    item += '<td>\
            <input class="disabled" type="text" value="'+purchaseno2+'" >\
            <input class="disabled" type="hidden" name="purchaseno[]" value="'+purchaseno+'" >\
            </td>';
    ...
    // delivery cost
    item += '<td>\
            <input class="disabled duit" type="text" name="delCost[]" value="'+parseFloat(deliverycost)+'" onkeyup="transaction_cek()">\
            </td>';
...
function get_detail_receipt(id){
    url = host + "Penerimaan/ajax_edit/"+id;
    data_post = {
        page : "invoice_receipt",
    }
    $.ajax({
        url : url,
        data : data_post,
