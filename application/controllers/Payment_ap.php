...
class Payment_ap extends CI_Controller {
	var $title = 'Payable Payment';
	public function __construct()
	{
		parent::__construct();
		$this->load->helper('url');
		$this->load->model("M_payment_ap",'payment');
		$this->main->cek_session();
		$this->title = $this->lang->line('lb_paymentap');
	}
	...
	{	
		...
		$ar 	= $this->main->check_parameter_module("ap","payment_ap");
		...
		$data['tambah'] 		= $tambah;
		$data['modal'] 			= 'payment_ap/modal';
		$data['modal_vendor'] 	= 'modal/modal_vendor';
		$data['page'] 			= 'payment_ap/list';
		$data['modul'] 			= 'payment_ap';
		$data['url_modul'] 		= $url;
		$data['ID']				= $ID;
		$data['Status']			= $Status;
	...
	public function save(){
		...
		if($crud == "update"):
			...
			$code = $this->main->payment_ap_generate();
			$data['PaymentNo'] 	= $code;
			$data['Status']		= 1;
		...
		// generate journal
		$this->db->query("CALL run_generate_jurnal('UPDATE', '$code', 'paymentap', '$CompanyID')");
	...
	private function validate(){
		...
		if($this->input->post('CustomerID') == ''):
			...
			$data['error_string'][] = $this->lang->line('lb_vendor_empty');
			$data['list'][] 		= '';
	...
	private function check_payment_status($invoiceno,$totalpay){
		...
		$query = $this->db->query("
            ...
            where ps_pd.CompanyID = '$CompanyID' and ps_pd.InvoiceNo = '$invoiceno' and ps_p.Status = 1 and ps_p.Type = 2
            ");
	...
	private function check_koreksi_status($balancedetid,$totalpay){
		...
		$query = $this->db->query("
            SELECT ifnull(sum(ps_pd.Total), 0) as Total from PS_Payment_Detail as ps_pd left join PS_Payment as ps_p 
            on ps_pd.PaymentNo = ps_p.PaymentNo and ps_pd.CompanyID = ps_p.CompanyID
            where ps_pd.CompanyID = '$CompanyID' and ps_pd.BalanceDetID = '$balancedetid' and ps_p.Status and ps_p.Type = 2
            ");
		$totalpaid  = (float) $query->row()->Total;
		$totalpay 	= (float) $this->main->checkDuitInput($totalpay);
		$this->db->where("CompanyID", $CompanyID);
	...
	public function cetak($id){
		$idnya 		= $id;
		$id 		= str_replace("-", "/", $id);
		$this->main->default_template("payment_ap");
		...
		$data['list']			= $list;
		$data["detail"]			= $detail;
		$data['title']  		= 'print payment payable';
    	...
    	if($page == "print"):
    		$data['template'] 	= $this->main->get_default_template("payment_ap");
    		$this->load->view('payment_ap/template',$data);
    	else:
    		$this->load->view('payment_ap/view',$data);
    	endif;
