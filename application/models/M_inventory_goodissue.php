...
class M_inventory_goodissue extends CI_Model {
    ...
    {   
        $this->db->where("PS_Correction.Type",4);
        $this->db->where("PS_Correction.CompanyID",$this->session->CompanyID);
        $this->db->from($this->table);
    ...
    public function serial_number($id,$det,$ProductID){
        ...
        $this->db->where("CorrectionDetID", $det);
        $this->db->where("ProductID", $ProductID);
        $query = $this->db->get("PS_Correction_Detail_SN");
