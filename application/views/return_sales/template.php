<?php
  ...
  foreach ($detail as $k => $v) {
    ...
    if(in_array("GridDeliveryCode", $hastag)): $item .= '<td>'.$list->transactionCode.'</td>'; endif;
    if(in_array("GridDeliveryDate", $hastag)): $item .= '<td>'.date("Y-m-d", strtotime($list->transactionDate)).'</td>'; endif;
    ...
    if(in_array("GridDeliveryQty", $hastag)): $item .= '<td>'.$this->main->qty($v->transactionQty).'</td>'; endif;
