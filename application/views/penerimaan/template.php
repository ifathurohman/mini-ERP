<?php
  ...
  $content = str_replace("##TransactionCode", $list->receipt_no, $content);
  $content = str_replace("##TransactionDate", $list->receipt_date, $content);
  $content = str_replace("##PurchaseOrderCode", $list->po_no, $content);
  $content = str_replace("##Term", $list->Term, $content);
  $content = str_replace("##DueDate", date("Y-m-d", strtotime($list->receipt_date." +".$list->Term." Days")), $content);
  $content = str_replace("##DeliveryDate", $list->DeliveryDate, $content);
  $content = str_replace("##SalesName", $list->salesName, $content);
  $content = str_replace("##VendorName", $list->receipt_name, $content);
  $content = str_replace("##VendorAddress", $list->vendorAddress, $content);
  $content = str_replace("##VendorContact", $list->vendorPhone, $content);
  $content = str_replace("##Remark", $list->receipt_remark, $content);
  $content = str_replace("##DeliveryOrderCode", $list->sj_no, $content);
  
  // untuk bentuk uang
  $content = str_replace("##SubTotal", $this->main->currency($list->Total), $content);
  $content = str_replace("##DiscountTotal", $this->main->currency($list->receipt_discount), $content);
  $content = str_replace("##TaxTotal", $this->main->currency($list->TotalPPN), $content);
  $content = str_replace("##DeliveryCost", $this->main->currency($list->receipt_cost), $content);
  ...
  foreach ($detail as $k => $v) {
    $no = $k + 1;
        ...
        if(in_array("GridOrderQty", $hastag)): $item .= '<td>'.$this->main->qty($qty,TRUE).'</td>'; endif;
        if(in_array("GridReceiptQty", $hastag)): $item .= '<td>'.$this->main->qty($qty2,TRUE).'</td>'; endif;
        if(in_array("GridProductUnit", $hastag)): $item .= '<td>'.$unit_name.'</td>'; endif;
        if(in_array("GridPriceUnit", $hastag)): $item .= '<td>'.$this->main->currency($v->receipt_price).'</td>'; endif;
        if(in_array("GridProductConv", $hastag)): $item .= '<td>'.$product_conv.'</td>'; endif;
        if(in_array("GridDiscountPersent", $hastag)): $item .= '<td>'.$this->main->currency($v->receipt_discount,TRUE).'</td>'; endif;
        if(in_array("GridDiscountValue", $hastag)): $item .= '<td>'.$this->main->currency($discount).'</td>'; endif;
        if(in_array("GridTax", $hastag)): $item .= '<td>'.$tax.'</td>'; endif;
        if(in_array("GridSubTotal", $hastag)): $item .= '<td>'.$this->main->currency($v->receipt_subtotal).'</td>'; endif;
